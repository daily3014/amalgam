...
namespace Vars
{
	...
	NAMESPACE_BEGIN(Aimbot)
		...
		SUBNAMESPACE_BEGIN(Projectile)
			...
			CVar(AutoDetLowHealthPercentage, 0.4f)
			...
			CVarEnum(RocketSplashMode, 0, NONE, SpecialHeavy, SpecialLight, Regular)
	...
	NAMESPACE_BEGIN(Visuals)
		...
		SUBNAMESPACE_BEGIN(Simulation)
			...
			CVar(ProjectileCameraDistance, 130.f, VISUAL)
			CVar(ProjectileWindow, WindowBox_t(), NOBIND)
			CVar(SeparatorSpacing, 4)
			CVar(SeparatorLength, 12)
			CVar(LineThickness, 1, DEBUGVAR)
			CVar(DoEvenBullshit, true, DEBUGVAR)
			CVar(OutlineColor, Color_t(0, 0, 0, 200), DEBUGVAR)
	...
	NAMESPACE_BEGIN(Colors)
		...
		CVar(SplashRadiusTeam, Color_t(255, 255, 255, 255), VISUAL)
		CVar(SplashRadiusEnemy, Color_t(255, 255, 255, 0), VISUAL)
